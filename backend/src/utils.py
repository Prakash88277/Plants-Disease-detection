...
def compute_gradcam(model,
    ...
    Algorithm
    ---------
    1. Build a sub-model that outputs (last_conv_features, final_predictions)
    2. Record gradients of the predicted class score w.r.t. the last conv output
    3. Pool gradients spatially → importance weights
    4. Weighted combination of feature maps → raw heatmap
    5. ReLU + normalise to [0, 1]

    Parameters
    ----------
    model                : keras.Model
    img_array            : np.ndarray shape (1, H, W, 3) – preprocessed image
    last_conv_layer_name : str – name of the last convolutional layer
    pred_index           : int – class index to explain; uses argmax if None
    ...
    print("GradCAM using:", last_conv_layer.name)

    # STEP 3: BUILD CORRECT GRAD MODEL
    from tensorflow.keras.models import Model

    grad_model = Model(
        inputs=model.input,
        outputs=[
            last_conv_layer.output,   # from EfficientNet
    ...
    # STEP 5: CONTINUE STANDARD GRADCAM
...
def overlay_gradcam(image_path: str,
    ...
    Returns
    -------
    np.ndarray (H, W, 3) uint8 – blended image
    """
    ...
    heatmap_color   = cm.jet(heatmap_uint8)[:, :, :3]          # RGBA → RGB
    heatmap_color   = np.uint8(255 * heatmap_color)

    blended = np.uint8(img * (1 - alpha) + heatmap_color * alpha)
    return blended
...
def show_gradcam(model,
                 ...
                 save_path:           str   = None):
    """
    End-to-end Grad-CAM visualisation: predict + heatmap + overlay.

    Parameters
    ----------
    model                : keras.Model
    image_path           : str – leaf image path
    class_names          : list
    last_conv_layer_name : str – e.g. "top_conv" for EfficientNet,
    ...
    # ── Predict & compute Grad-CAM ──────────────────────────────────────
    preds       = model.predict(inp, verbose=0)[0]
    pred_idx    = int(np.argmax(preds))
    pred_conf   = float(preds[pred_idx])
    pred_cls    = class_names[pred_idx]
    plant, dis  = pred_cls.split("___", maxsplit=1) if "___" in pred_cls else (pred_cls, "N/A")

    heatmap     = compute_gradcam(model, inp, last_conv_layer_name, pred_idx)
    overlay     = overlay_gradcam(image_path, heatmap)

    # ── Plot ────────────────────────────────────────────────────────────
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle(
        f"Grad-CAM  |  {plant} – {dis.replace('_',' ')}  "
        f"(conf={pred_conf:.2%})",
    ...
    if save_path:
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
        print(f"[utils] Grad-CAM saved → {save_path}")
    plt.show()
